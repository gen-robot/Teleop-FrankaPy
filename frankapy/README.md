...
* A computer with the Ubuntu 18.04 / 20.04 / 22.04.
* ROS Melodic / Noetic or ROS2 Humble 
...
1. Clone Repo and its Submodules:
   ...
   ```bash
   git clone --recurse-submodules git@github.com:iamlab-cmu/frankapy.git # we dont use submodule
   ```
   All directories below are given relative to `/frankapy`.

2. To allow asynchronous gripper commands, we use the franka\_ros package, so install libfranka and franka\_ros using the following command (Change melodic to noetic if you are on Ubuntu 20.04):
   ```bash
...
5. [**Optional**] To make the protobufs use the following script (**you don't need to do this if you haven't modified the proto messages**):
	```bash
...
```
### Using autolab_core
- you should check the version of `catkin_ws/src/autolab_core/package.xml` and `catkin_ws/src/autolab_core/autolab_core/version.py`. modify them as the same version.
